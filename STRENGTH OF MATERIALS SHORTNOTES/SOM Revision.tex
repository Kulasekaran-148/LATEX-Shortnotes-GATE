...
%-----------------------------------------------------------------------------------------%
	...
	\subsection{Member subjected to Uni-axial stress}
		\begin{itemize}
			\begin{figure}[H]
				\centering
			...
			\item By resolution of $P_x$, $\boxed{P_n = P_xSin(90-\theta) = P_xCos\theta}\;\;\;\boxed{P_t = P_xCos(90-\theta) = P_xSin\theta}$
			\item $Cos\theta = \dfrac{A_{abcd}}{A_{abwx}} \implies \boxed{A_{abwx} = \dfrac{A_{abcd}}{Cos\theta}}$
		  \item So, The normal stress ($\sigma_{ob}$) acting on the plane (abwx) is given by: $$\boxed{\sigma_{ob} = \dfrac{P_n}{A_{abwx}} = \dfrac{P_xCos\theta}{\sfrac{A_{abcd}}{Cos\theta}} = \dfrac{P_xCos^2\theta}{A_{abcd}} \implies \boxed{\sigma_{ob} = \sigma_{x}Cos^2\theta}}$$
		  \item $\sigma_{ob}$ is max when $\theta$ is 0\textdegree , 180\textdegree ... $\impliedby$ (Because at these angles Cos is max)
		  \item Similarly, Tangential stress ($\tau_{ob}$) acting along the plane (abwx) is given by: $$\boxed{\tau_{ob} = \dfrac{P_t}{A_{abwx}} = \dfrac{P_xSin\theta}{\sfrac{A_{abcd}}{Cos\theta}} = \dfrac{P_xSin\theta Cos\theta}{A_{abcd}} \implies \boxed{\tau_{ob} = \dfrac{\sigma_x}{2}Sin2\theta}}$$
		  \item $\tau_{ob}$ is max when $\theta$ is 45\textdegree , 135\textdegree ... $\impliedby$ (Because at these angles Sin is max)
		\end{itemize}\hrulefill
%-----------------------------------------------------------------------------------------%
	\subsection{Member subjected to Bi-axial stress in mutually perpendicular direction}
		\begin{itemize}
			\begin{figure}[H]
				\centering
				\includegraphics[scale=0.5]{case2.png}
			\end{figure}
			\item From Diagram(2) above, $\boxed{\sin\theta = \dfrac{A_{dcwx}}{A_{abwx}}}_{(1)}\;\;\;\boxed{\cos\theta = \dfrac{A_{abcd}}{A_{abwx}}}_{(2)}$	
